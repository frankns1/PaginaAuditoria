...
public class UsuarioServlet extends HttpServlet {
    ...
    protected void doPostRequestLogin(HttpServletRequest request, HttpServletResponse response)
        ...
        Usuario usuario_auth = null;
        try {
            usuario = obtenerUsuario(request);
            usuario_auth = UsuarioDAL.login(usuario);//Cambiar esta linea
            ...
            if (usuario != null) {
                System.out.println("Intentando login para usuario: " + usuario.getLogin());
            } else {
                System.out.println("El objeto usuario es NULL");
            }
            if (usuario_auth != null) {
                System.out.println("Resultado ID devuelto: " + usuario_auth.getId());
            } else {
                System.out.println("El objeto usuario_auth es NULL");
            }

            ex.printStackTrace();// 🔥 Muestra el error exacto con línea y clase
            
            request.setAttribute("error", "Error al procesar el inicio de sesión: " + ex.getMessage());
            request.setAttribute("accion", "login");
            doGetRequestLogin(request, response);
        }
    }
