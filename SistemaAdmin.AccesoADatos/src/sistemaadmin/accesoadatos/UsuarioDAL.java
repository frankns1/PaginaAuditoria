...
import sistemaadmin.entidades.Usuario;
import sistemaadmin.entidades.Rol;
...
public class UsuarioDAL {
    ...
    private static boolean existeLogin(Usuario pUsuario) throws Exception {
        ...
        try (Connection conn = ConexionDB.obtenerConexion();) { // Obtener la conexion desde la clase ConexionDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ConexionDB.createPreparedStatement(conn, sql);) { // Obtener el PreparedStatement desde la clase ConexionDB
            ...
            throw ex; // Enviar al siguiente metodo el error al obtener la conexion  de la clase ConexionDB en el caso que suceda
        }
    ...
    public static int modificar(Usuario pUsuario) throws Exception {
        ...
        if (existe == false) {
            ...
            result = 0;
            throw new RuntimeException("El Usuario ya existe"); // enviar una exception para notificar que el login existe
        }
    ...
    public static int eliminar(Usuario pUsuario) throws Exception {
        ...
        try (Connection conn = ConexionDB.obtenerConexion();) { // Obtener la conexion desde la clase ConexionDB y encerrarla en try para cierre automatico
            ...
            try (PreparedStatement ps = ConexionDB.createPreparedStatement(conn, sql);) {  // obtener el PreparedStatement desde la clase ComunDB
    ...
    private static void obtenerDatos(PreparedStatement pPS, ArrayList<Usuario> pUsuarios) throws Exception {
        try (ResultSet resultSet = ConexionDB.obtenerResulSet(pPS);) { // obtener el ResultSet desde la clase ConexionDB
    ...
    static void querySelect(Usuario pUsuario, ConexionDB.UtilQuery pUtilQuery) throws SQLException {
    ...
    public static ArrayList<Usuario> buscar(Usuario pUsuario) throws Exception {
        ...
        try (Connection conn = ConexionDB.obtenerConexion();) { // Obtener la conexion desde la clase ConexionDB y encerrarla en try para cierre automatico
            ...
            ConexionDB.UtilQuery utilQuery = conexiondb.new UtilQuery(sql, null, 0);
    ...
    public static int cambiarPassword(Usuario pUsuario, String pPasswordAnt) throws Exception {
        ...
        if (usuarioAut.getId() > 0 && usuarioAut.getLogin().equals(pUsuario.getLogin())) {
            try (Connection conn = ConexionDB.obtenerConexion();) { // Obtener la conexion desde la clase ComunDB y encerrarla en try para cierre automatico
            ...
            throw new RuntimeException("La Contraceña actual es incorrecto");
        }
        return result; // Retornar el numero de fila afectadas en el UPDATE en la base de datos 
    }
    ...
    public static ArrayList<Usuario> buscarIncluirRol(Usuario pUsuario) throws Exception {
        ...
        try (Connection conn = ConexionDB.obtenerConexion();) { // Obtener la conexion desde la clase ConexionDB y encerrarla en try para cierre automatico
            ...
            if (pUsuario.getTop_aux() > 0 && ConexionDB.TIPODB == ConexionDB.TipoDB.SQLSERVER) {
